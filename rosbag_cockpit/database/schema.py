...
class DatabaseManager:
    ...
    def insert_rosbag_metadata(self, metadata: RosbagMetadata) -> None:
        ...
        valid_columns = [col for col in columns if col in metadata_dict or col == 'id' or col == 'created_at']
        
        column_names = ', '.join([col for col in valid_columns if col != 'id' and col != 'created_at'])
        placeholders = ', '.join(['?'] * (len(valid_columns) - 2))  # Exclude id and created_at
        
        values = [metadata_dict.get(col) for col in valid_columns if col != 'id' and col != 'created_at']
    ...
    def print_database_stats(self) -> None:
        ...
        self.cursor.execute("SELECT map_category, COUNT(*) FROM rosbags GROUP BY map_category")
