<template>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div class="bg-white shadow-md rounded-lg p-6">
        ...
        <div v-else class="grid grid-cols-2 gap-4">
          ...
          <div class="bg-yellow-50 p-4 rounded-lg">
            ...
            <p class="text-lg font-bold">{{ stats.latestUpload ? formatDate(stats.latestUpload) : 'None' }}</p>
          </div>
        </div>
      </div>
            ...
            <li v-for="(activity, index) in activities" :key="index" class="py-3">
              <div class="flex items-start">
                <div class="flex-shrink-0">
                  <span :class="getActivityIcon(activity.type).bgColor" class="flex items-center justify-center h-8 w-8 rounded-full">
                    <svg class="h-5 w-5" :class="getActivityIcon(activity.type).textColor" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="bg-white shadow-md rounded-lg p-6 md:col-span-2 lg:col-span-1">
        ...
        <div v-else>
          <ul class="space-y-3">
            <li v-for="(topic, index) in popularTopics" :key="index" class="bg-gray-50 rounded-lg p-3">
              ...
              <div class="mt-2 w-full bg-gray-200 rounded-full h-2.5">
                <div class="bg-blue-600 h-2.5 rounded-full" :style="{ width: `${(topic.count / popularTopics[0].count) * 100}%` }"></div>
              </div>
            </li>
          </ul>
        </div>
